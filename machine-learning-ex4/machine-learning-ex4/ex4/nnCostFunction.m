...
% 
% finaldelta1 = zeros(size(Theta1));
% finaldelta2 = zeros(size(Theta2));
% 
% for t=1:m
%     a1t = a1(t,:)';
%     %size(a1t)
...
%     %size(delta3)
%     z2t = [1; Theta1 * a1t];
%     delta2 = ((Theta2)' * delta3) .* (sigmoidGradient(sigmoid(z2t)));
%     delta2 = delta2(2:end);
%     %size(delta2)
%     
%     finaldelta1 = finaldelta1 + delta2 * (a1t)';
%     finaldelta2 = finaldelta2 + delta3 * (a2t)';
%     
% end
% 
% 
% Theta1_grad = (1 / m) * finaldelta1 + (lambda/m)*[zeros(size(Theta1, 1), 1) Theta1(:,2:end)];
% Theta2_grad = (1 / m) * finaldelta2 + (lambda/m)*[zeros(size(Theta2, 1), 1) Theta1(:,2:end)];
